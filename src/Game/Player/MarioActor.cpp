...
void MarioActor::updateSwingAction()
{
    ...
    u8 action = selectAction("スピンアタック"); // Spin attack
    switch (action) {
    case 1:
    ...
    case 5:
        ...
        if (isJumping()) {
            mMario->changeAnimation("ハチスピン空中", (const char *)nullptr); // Midair spin
        }
        else {
            if (getMovementStates()._A || mAlphaEnable) {
                ...
                mMario->changeAnimation("ハチスピン", (const char *)nullptr); // Spin
            }
        }
        _946 = mConst->_0[mConst->_8]->_426 + 0x22;
        break;
