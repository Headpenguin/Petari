...
Poihana::Poihana(const char *pName) : LiveActor(pName) {
	...
	mRespawnPos.setInline(0.0f, 0.0f, 0.0f);
	_AC = 0.0f;
	_B0 = 0.0f;
	mHomePos.setInline(0.0f, 0.0f, 0.0f);
	_C0 = 1.0f;
	mBoundTimer = -1;
	...
	if (MR::isSensorEnemy(pSender)) {
		...
		if (MR::sendMsgPush(pReceiver, pSender)) {
			...
			if (mVelocity.dot(pushVelocity) < 0.0f) {
				...
				f32 dot = pushVelocity.dot(velocity);
				//f32 dot = -(pushVelocity.dot(mVelocity));
				//scaleAlt(pushVelocity.toCVec(), velocity, dot);
				mVelocity.MAVECScaleAdd2(pushVelocity, velocity, dot);
			}
		}
	}
}
...
void Poihana::exeRecover() {
	if (MR::isFirstStep(this)) {
		...
		mScale.setInline(1.0f, 1.0f, 1.0f);
	}
...
/*bool Poihana::tryToStartBind(HitSensor* pSender) {
	if (mBindedActor == nullptr && !MR::isInWater(this, TVec3f(0.0f, 0.0f, 0.0f))) {
...
/*void Poihana::contactMario(HitSensor *pSender, HitSensor *pReceiver) {
	...
	if (ret) {
		...
		if (MR::sendMsgEnemyAttackFlipWeak(pSender, pReceiver)) {
